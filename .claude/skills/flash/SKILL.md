...
Build and flash a firmware build to the ESP32-C3 Super Mini.

`$ARGUMENTS` may name the build and/or target:
- Build: `standalone` (default) → `aurora-projector.yaml`; `ha` → `aurora-projector-ha.yaml`.
- Target: `usb` (default) → `--device /dev/ttyACM0`; `ota`/`network` → no `--device` flag;
  or an explicit IP/hostname → `--device <value>`.

The C3 Super Mini uses native USB, so it enumerates as `/dev/ttyACM0` (not
`/dev/ttyUSB0`). If it isn't there, the board may be in the wrong USB mode — hold
...
3. For a USB flash, confirm the board is present (`ls /dev/ttyACM0`) and that **no
...
4. Run the flash (long-running; allow several minutes for the first compile):
   - USB: `esphome run <entrypoint>.yaml --device /dev/ttyACM0`
